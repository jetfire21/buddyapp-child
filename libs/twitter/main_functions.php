...
class Group_Extension_Example_2 extends BP_Group_Extension {
    ...
    function settings_screen( $group_id = NULL ) {
        ...
        ?>
        Twitter url: 
        <input type="text" name="al21_twitteer_url" value="<?php echo esc_attr( $setting ) ?>" />
		<label for="al21_automate_enable">
		<input type="checkbox" id="al21_automate_enable" name="al21_automate_enable" <?php if($setting_2 == 'yes') echo 'checked="checked"';?>value="yes"> Enable automate
		</label>
...
function al_add_tweets_in_db(){
	...
	if (!empty($twitter_url) ){
		...
		$tweets = a21_tw_get_tweets($twitter_username,$settings,$url,$requestMethod,$twitter_debug);
		...
		if(!$twitter_debug):
			foreach ($tweets as $k => $v):
				...
				echo $date_recorded = date("Y-m-d H:i:s", strtotime($output['date']));
		...
		print_r($output);
		// INSERT INTO `wp8k_bp_activity` (`id`, `user_id`, `component`, `type`, `action`, `content`, `primary_link`, `item_id`, `secondary_item_id`, `date_recorded`, `hide_sitewide`, `mptt_left`, `mptt_right`, `is_spam`) VALUES (NULL, '1', 'groups', 'new_event', '<a href="http://dugoodr.dev/members/toddroberts/" title="Todd2_LongName">Todd2_LongName</a> posted tweet <a href="http://dugoodr.dev/causes/ottawa-food-bank/">Ottawa Food Bank</a>', 'just tweets', 'http://dugoodr.dev/members/toddroberts/', '13', '0', '2017-03-02 21:59:30', '0', '0', '0', '0')
	}
	echo "<br>end twitter";
	 echo '<hr><br>';
